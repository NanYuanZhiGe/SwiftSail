...
public class ReportDetailCommonFragment extends BackPressQuitFragment {
    ...
    private static final String[] MONTH_X_LABEL = new String[]{"第一周", "第二周", "第三周", "第四周", "第五周", "第六周"};
    ...
    public static Fragment getInstance(
            int themeColor,
            Function<FuncParam, Pair<Long, List<BarEntry>>> logicFunc,
            String funcKey,
            SumType sumType,
            Function<Float, String> averageFormatter) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        recyclerView = father.findViewById(R.id.avgList);
        recyclerView.setNestedScrollingEnabled(false);
        recyclerView.setAdapter(new MyRecyclerAdapter(sumType, avgFormatter));
    ...
    private void notifyBarChartDatasetChange(long offset, List<BarEntry> entries) {
        ...
        //第一次执行
        Log.v("myTag", "Are you kidding me?");
        barDataSet = new BarDataSet(entries, "");
        Log.v("myTag", "Anything wrong?");
        ...
        try {
            ...
            Log.v("myTag", Arrays.toString(e.getStackTrace()));
            return;
        }
        Log.v("myTag", "You can create BarData?");
        ...
        Log.v("myTag", "Can you run at here?");
    ...
    private void notifyTextViewChange(Float avg) {
        ...
        float largeSizePx = TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP,
                32, //sp
                getResources().getDisplayMetrics()
        );
        spannable.setSpan(
                new AbsoluteSizeSpan((int) largeSizePx),
                start,
                end,
    ...
    private void doLogicFunc(FuncParam funcParam) {
        ...
        Log.v("myTag", "doLogicFunction");
        FUNC_CACHE.computeIfPresent(funcKey, (k, v) -> {
            Log.v("myTag", "funcKeyPresent-" + funcKey);
            Function<FuncParam, Pair<Long, List<BarEntry>>> function = v.getB();
            CompletableFuture
                    .supplyAsync(() -> function.apply(funcParam), THREAD_POOL)
    ...
    public static class MyRecyclerAdapter extends RecyclerView.Adapter<MyViewHolder> {
        ...
        final private Function<Float, String> avgFormatter;

        public MyRecyclerAdapter(SumType sumType, Function<Float, String> avgFormatter) {
            this.sumType = sumType;
            this.avgFormatter = avgFormatter;
        }
        ...
        public MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            return new MyViewHolder(LayoutInflater.from(parent.getContext()).inflate(
                    R.layout.layout_report_detail_sum, parent, false
            ));
        }
        ...
        public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
            ...
            if (sumType == SumType.WEEK) {
                holder.index.setText(String.format("周%s", WEEK_X_LABEL[index]));
            } else if (sumType == SumType.MONTH) {
                holder.index.setText(MONTH_X_LABEL[index]);
            } else if (sumType == SumType.YEAR) {
                holder.index.setText(YEAR_X_LABEL[index]);
            } else if (sumType == SumType.TOTAL) {
                holder.index.setText(String.format("%d年", offset + index));
            }
            //平均数（加上单位），比如9小时2分钟，3600（步数）之类的
            holder.detail.setText(avgFormatter.apply(barEntries.get(index).getY()));
            //日期范围
            ...
            if (sumType == SumType.WEEK) {
                ...
                holder.date.setText(LocalDate.ofEpochDay(date).format(DATE_TIME_FORMATTER));
            } else if (sumType == SumType.MONTH) {
                //需要计算第几周，注意逆序
                ...
                long passWeek = position * 7L;//相比于offset，加了position*7天的时间
                ...
                holder.date.setText(String.format("%s-%s",
                        LocalDate.ofEpochDay(weekStart).format(DATE_TIME_FORMATTER),
                        LocalDate.ofEpochDay(weekEnd).format(DATE_TIME_FORMATTER)));
            } else if (sumType == SumType.YEAR) {
                //需要计算第几月，注意逆序
                ...
                holder.date.setText(String.format("%s-%s",
                        startMonth.format(DATE_TIME_FORMATTER),
                        LocalDate.ofEpochDay(monthEnd).format(DATE_TIME_FORMATTER)));
            } else if (sumType == SumType.TOTAL) {
                long year = offset + index;
                holder.date.setText(String.format("%s年", year));
            }
        }
